...
class ClassifierDemo {
  ...
  public static void main(String[] args) throws Exception {
    ColumnDataClassifier cdc = new ColumnDataClassifier("source/classifier/cheese2007.prop");
    Classifier<String,String> cl =
        cdc.makeClassifier(cdc.readTrainingExamples("source/classifier/cheeseDisease.train"));
    for (String line : ObjectBank.getLineIterator("source/classifier/cheeseDisease.test", "utf-8")) {
  ...
  public static void demonstrateSerialization()
    ...
    ois.close();
    ColumnDataClassifier cdc2 = new ColumnDataClassifier("source/classifier/cheese2007.prop");

    // We compare the output of the deserialized classifier lc versus the original one cl
    // For both we use a ColumnDataClassifier to convert text lines to classifier
    for (String line : ObjectBank.getLineIterator("source/classifier/cheeseDisease.test", "utf-8")) {
      Datum<String,String> d = cdc.makeDatumFromLine(line);
